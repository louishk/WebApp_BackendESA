# Pricing Recalibration Script — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a Python script that outputs a CSV with per-unit current pricing vs recalibrated targets based on budget ARR, a -log yield curve, and a climate matrix.

**Architecture:** Single standalone script (`scripts/pricing_recalibration.py`) that connects to `esa_pbi` via SQLAlchemy, runs one big query to get all unit data, computes the yield curve and recalibrated rates in Python, and writes a CSV. No new dependencies — uses stdlib `math` and `csv` plus existing `sqlalchemy` and `common.config_loader`.

**Tech Stack:** Python 3, SQLAlchemy (raw SQL via `text()`), stdlib `math`/`csv`

**Spec:** `docs/superpowers/specs/2026-03-30-pricing-recalibration-design.md`
...
| File | Responsibility |
|------|---------------|
| `backend/python/scripts/pricing_recalibration.py` | Main script — config, DB query, yield curve fit, CSV output |
| `output/` | Output directory for generated CSVs (gitignored) |

This is a single-file script. No modules, no abstractions — matches the project's existing `scripts/` pattern (see `populate_unit_range_mappings.py` for reference).

---

### Task 1: Scaffold script with config and DB connection

**Files:**
- Create: `backend/python/scripts/pricing_recalibration.py`
- Create: `output/.gitkeep`
- Create: `output/.gitignore`

- [ ] **Step 1: Create output directory with .gitkeep and .gitignore**
...
- [ ] **Step 2: Write the script scaffold with configurable parameters and DB connection**
...
def main():
    ...
    # TODO: Task 3 — fit yield curves and compute recalibrated rates
    # TODO: Task 4 — write CSV
...
- [ ] **Step 3: Test the scaffold connects to DB**

```bash
cd /home/louis/PycharmProjects/WebApp_BackendESA/backend/python
python -m scripts.pricing_recalibration --dry-run
```

Expected: `INFO: Connected to esa_pbi` — no errors.

- [ ] **Step 4: Commit**

```bash
git add backend/python/scripts/pricing_recalibration.py output/.gitkeep output/.gitignore
git commit -m "feat: scaffold pricing recalibration script with config and DB connection"
```

---

### Task 2: Fetch all unit data in a single query

**Files:**
- Modify: `backend/python/scripts/pricing_recalibration.py`

The query joins rentroll_enriched + unit_category_labels + siteinfo + vw_budget_monthly and returns everything needed. Includes a window function for multi-tenancy detection. One query, no ORM.

- [ ] **Step 1: Add the data fetch function**
...
Expected: `INFO: Fetched XXXXX units across NN sites` — confirms query works.

- [ ] **Step 4: Commit**

```bash
git add backend/python/scripts/pricing_recalibration.py
git commit -m "feat: add data fetch query joining rentroll, labels, siteinfo, budget"
```

---

### Task 3: Implement yield curve fitting and recalibration

**Files:**
- Modify: `backend/python/scripts/pricing_recalibration.py`

The curve fit groups units by site + size_range, computes NLA-weighted avg book rate per bucket, then fits `rate = a - b * ln(area)` using weighted least squares. Then applies climate multipliers and rescales to budget. Pure stdlib math.

**Important:** For multi-tenant units (multiple rows per unit_id), each row's NLA contribution is divided by `tenant_count` to avoid double-counting area in site totals.

- [ ] **Step 1: Add the yield curve fitting function**

Add after `fetch_data()`:
...
- [ ] **Step 2: Add the recalibration function**
...
def compute_recalibrated(rows, curves):
    ...
    for site_id, st in sorted(site_totals.items()):
        ...
        a, b_coeff = curves.get(site_id, (0, 0))
        logger.info(
...
Expected:
- Yield curve coefficients per site (b > 0 for all)
- Per-site validation: `residual ~0.0000%` confirming budget match
...
```bash
git add backend/python/scripts/pricing_recalibration.py
git commit -m "feat: implement yield curve fitting and budget-rescaled recalibration"
```

---

### Task 4: Write CSV with all derived columns

**Files:**
- Modify: `backend/python/scripts/pricing_recalibration.py`

Compute all derived columns from raw values, apply rounding as the very last step before writing.

- [ ] **Step 1: Add the CSV writing function**

Add after `compute_recalibrated()`:
...
- [ ] **Step 3: Run full script to generate CSV**
...
Expected: `CSV written: .../output/pricing_recalibration_2026-03-30.csv (XXXXX rows)`

- [ ] **Step 4: Spot-check the CSV**

```bash
head -1 output/pricing_recalibration_2026-03-30.csv  # verify all headers present
wc -l output/pricing_recalibration_2026-03-30.csv     # row count matches fetched units
```
...
```bash
git add backend/python/scripts/pricing_recalibration.py
git commit -m "feat: complete pricing recalibration CSV with all derived columns and rounding"
```
