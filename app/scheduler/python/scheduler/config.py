...
class SchedulerConfig:
    ...
    def from_env(cls) -> 'SchedulerConfig':
        ...
        # Daemon
        config.daemon.pid_file = env_config('SCHEDULER_PID_FILE', default=config.daemon.pid_file)
        config.daemon.log_file = env_config('SCHEDULER_LOG_FILE', default=config.daemon.log_file)

        # Resources
        config.resources.db_pool = env_config('SCHEDULER_DB_POOL', default=config.resources.db_pool, cast=int)
        config.resources.soap_api = env_config('SCHEDULER_SOAP_SLOTS', default=config.resources.soap_api, cast=int)
        config.resources.http_api = env_config('SCHEDULER_HTTP_SLOTS', default=config.resources.http_api, cast=int)

        # Engine
        config.timezone = env_config('SCHEDULER_TIMEZONE', default=config.timezone)
        config.executor_max_workers = env_config('SCHEDULER_MAX_WORKERS', default=config.executor_max_workers, cast=int)

        # Web UI
        config.web_host = env_config('SCHEDULER_WEB_HOST', default=config.web_host)
        config.web_port = env_config('SCHEDULER_WEB_PORT', default=config.web_port, cast=int)

        # Slack alerts
        slack_url = env_config('SLACK_WEBHOOK_URL', default='')
        if slack_url:
            config.alerts.slack = SlackConfig(
                enabled=True,
                webhook_url=slack_url,
                channel=env_config('SLACK_CHANNEL', default='#data-pipeline-alerts'),
            )
...
def _resolve_env(value: str) -> str:
    ...
    def replace(match):
        ...
        return env_config(var_name, default='')
