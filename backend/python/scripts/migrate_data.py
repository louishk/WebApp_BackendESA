...
Environment variables required:
    ...
    - Target DB (backend): DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD (from root .env)
"""
...
def get_target_engine():
    ...
    url = (
        f"postgresql://{env_config('DB_USER')}:{env_config('DB_PASSWORD')}@"
        f"{env_config('DB_HOST')}:{env_config('DB_PORT', default=5432)}/{env_config('DB_NAME', default='backend')}"
        f"?sslmode={env_config('DB_SSLMODE', default='require')}"
    )
    return create_engine(url)
