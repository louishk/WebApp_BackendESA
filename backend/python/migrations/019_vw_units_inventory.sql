-- Recreate vw_units_inventory view on esa_pbi database.
-- Uses CREATE OR REPLACE so it is safe to re-run (idempotent).
...
ORDER BY
    up."UnitID",
    CASE WHEN rm.id IS NULL THEN 1 ELSE 0 END,
    (rm.range_end - rm.range_start),
    CASE WHEN rm.suffix_start IS NOT NULL THEN 0 ELSE 1 END;
