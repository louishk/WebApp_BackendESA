"""
Register sync_service.pipelines.mimo.MimoPipeline in mw_sync_pipelines.

Run from backend/python:
    python3 migrations/mw_seed_mimo.py
"""
import sys
...
def main():
    ...
    print('[1] Seeding mw_sync_pipelines row for mimo...')
    with mw_engine.begin() as conn:
        ...
        """), {
            'name': 'mimo',
            'display': 'Move-In/Move-Out',
            'desc': 'Fetch tenant move-in/move-out events via SMD SOAP report '
                    '"move_ins_and_move_outs" for all locations and upsert into '
                    'esa_pbi.mimo keyed on (SiteID, TenantID, MoveDate). Auto mode '
                    'deletes rows where MoveDate >= today-60d, then repushes '
                    'a [-60d, +365d] window.',
            'cls': 'sync_service.pipelines.mimo.MimoPipeline',
            'sched': '{"cron": "0 */4 * * *"}',
            'ttl': 5 * 3600,  # runs every 4h
            'args': '{"mode": "auto"}',
        })
    print('    done')
