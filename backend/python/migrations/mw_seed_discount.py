"""
Register sync_service.pipelines.discount.DiscountPipeline in mw_sync_pipelines.

Run from backend/python:
    python3 migrations/mw_seed_discount.py
"""
import sys
...
def main():
    ...
    print('[1] Seeding mw_sync_pipelines row for discount...')
    with mw_engine.begin() as conn:
        ...
        """), {
            'name': 'discount',
            'display': 'Discounts',
            'desc': 'Fetch discount/concession records via SMD SOAP report '
                    '"discounts" for all locations and upsert into esa_pbi.discount '
                    'keyed on (extract_date, SiteID, ChargeID). Current month '
                    'overwrites; closed months use EOM extract_date.',
            'cls': 'sync_service.pipelines.discount.DiscountPipeline',
            'sched': '{"cron": "30 6 * * *"}',
            'ttl': 23 * 3600,
            'args': '{"mode": "auto"}',
        })
