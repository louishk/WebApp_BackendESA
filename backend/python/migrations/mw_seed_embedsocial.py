"""
Register sync_service.pipelines.embedsocial.EmbedSocialPipeline in mw_sync_pipelines.

Run from backend/python:
    python3 migrations/mw_seed_embedsocial.py
"""
import sys
...
def main():
    ...
    print('[1] Seeding mw_sync_pipelines row for embedsocial...')
    with mw_engine.begin() as conn:
        conn.execute(text("""
            INSERT INTO mw_sync_pipelines (
                ...
                'embedsocial_reviews', 'synced_at', NULL,
                :ttl, 'pbi',
                1, 'http_api', 2,
                600, 3, 300,
                CAST(:args AS jsonb)
        ...
        """), {
            'name': 'embedsocial',
            'display': 'EmbedSocial Reviews',
            'desc': 'Sync Google reviews from EmbedSocial GetItems API into '
                    'esa_pbi.embedsocial_reviews keyed on review_id. Auto mode '
                    'paginates newest-first and stops once a page has zero new IDs.',
            'cls': 'sync_service.pipelines.embedsocial.EmbedSocialPipeline',
            'sched': '{"cron": "0 7 * * *"}',
            'ttl': 23 * 3600,
            'args': '{"mode": "auto"}',
        })
