"""
Register sync_service.pipelines.google_ads.GoogleAdsPipeline in
mw_sync_pipelines so the orchestrator picks it up.

Replaces the legacy APScheduler entry `google_ads` from config/pipelines.yaml.

Run from backend/python:
    python3 migrations/mw_seed_google_ads.py
"""
import sys
...
def main():
    ...
    print('[1] Seeding mw_sync_pipelines row for google_ads...')
    with mw_engine.begin() as conn:
        conn.execute(text("""
            INSERT INTO mw_sync_pipelines (
                ...
                'gads_campaign_daily', 'segments_date', NULL,
                :ttl, 'pbi',
                1, 'http_api', 2,
                1800, 3, 300,
                CAST(:args AS jsonb)
