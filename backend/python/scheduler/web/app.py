...
def create_app(config=None, db_url=None):
    ...
    def api_run_job_async(pipeline):
        """Trigger job execution asynchronously with streaming support."""
        from scheduler.config import SchedulerConfig
    ...
    def api_cleanup_stale():
        """Mark stale running jobs as failed (interrupted)."""
        from scheduler.models import JobHistory
...
def run_app(host='0.0.0.0', port=5000, debug=False, db_url=None):
    ...
    if not db_url:
        ...
        # Support both DB_* and POSTGRESQL_* variable names
        ...
        password = secure_config('DB_PASSWORD') or os.getenv('POSTGRESQL_PASSWORD')
