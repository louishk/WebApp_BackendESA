"""
CcwsInsuranceCoveragePipeline — raw InsuranceCoverageRetrieve_V2 → middleware.
"""
...
from sync_service.pipelines._ccws_utils import (
    ...
    to_int, to_decimal, build_upsert_sql, parallel_fetch,
)
...
def _transform(record: Dict[str, Any], site_code: str) -> Optional[Dict[str, Any]]:
    ic_id = to_int(record.get('InsurCoverageID'))
    site_id = to_int(record.get('SiteID'))
    if ic_id is None or site_id is None:
        return None
    return {
        'InsurCoverageID': ic_id,
        'SiteID': site_id,
        'SiteCode': site_code,
        'dcCoverage': to_decimal(record.get('dcCoverage')),
        'dcPremium': to_decimal(record.get('dcPremium')),
        'dcPCTheft': to_decimal(record.get('dcPCTheft')),
...
class CcwsInsuranceCoveragePipeline(BasePipeline):
    ...
    def _execute(self, scope: Dict[str, Any]) -> RunResult:
        ...
        try:
            ...
            sql = text(build_upsert_sql(
                'ccws_insurance_coverage', cols,
                conflict_cols=['InsurCoverageID', 'SiteID'],
            ))
