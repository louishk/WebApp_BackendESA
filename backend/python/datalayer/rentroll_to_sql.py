...
def _get_pipeline_config(pipeline_name: str, key: str, default=None):
    """Get pipeline-specific config from unified config system."""
    try:
        from common.config_loader import get_config
        config = get_config()
        pipeline_cfg = getattr(config.scheduler.pipelines, pipeline_name, None)
        if pipeline_cfg:
...
def push_to_database(
    ...
    with session_manager.session_scope() as session:
        ...
        if status == "current":
            ...
            if cleaned > 0:
                tqdm.write(f"  ✓ Cleaned {cleaned} stale non-EOM records for {year}-{month:02d}")
...
def main():
    ...
    if not location_codes:
        raise ValueError("RENTROLL location_codes not configured in scheduler.yaml")
